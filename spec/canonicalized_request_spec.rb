...
describe 'Escher' do
  fixtures.each do |test|
    it "should calculate canonicalized request for #{test}" do
      ...
      canonicalized_request = Escher.new.canonicalize method, url, body, date, headers, headers_to_sign, 'SHA256', 'Authorization'
  ...
  it 'should validate request' do
    ...
    expect(Escher.new.validate_request 'GET', 'http://host.foo.com/', '', headers, 'Authorization', 'Date', 'AWS4').to be true
  end
end
